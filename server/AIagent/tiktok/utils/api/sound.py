...
class Sound:
    ...
    def __extract_from_data(self):
        data = self.as_dict
        keys = data.keys() # type: ignore

        if "musicInfo" in keys:
            author = data.get("musicInfo").get("author") # type: ignore
            if isinstance(author, dict):
                self.author = self.parent.user(data=author)
            elif isinstance(author, str):
                self.author = self.parent.user(username=author)

            if data.get("musicInfo").get("music"): # type: ignore
                self.title = data.get("musicInfo").get("music").get("title") # type: ignore
                self.id = data.get("musicInfo").get("music").get("id") # type: ignore
                self.original = data.get("musicInfo").get("music").get("original") # type: ignore
                self.play_url = data.get("musicInfo").get("music").get("playUrl") # type: ignore
                self.cover_large = data.get("musicInfo").get("music").get("coverLarge") # type: ignore
                self.duration = data.get("musicInfo").get("music").get("duration") # type: ignore

        if "music" in keys:
            self.title = data.get("music").get("title") # type: ignore
            self.id = data.get("music").get("id") # type: ignore
            self.original = data.get("music").get("original") # type: ignore
            self.play_url = data.get("music").get("playUrl") # type: ignore
            self.cover_large = data.get("music").get("coverLarge") # type: ignore
            self.duration = data.get("music").get("duration") # type: ignore

        if "stats" in keys:
            self.stats = data.get("stats") # type: ignore
