...
class TikTokPlaywrightSession:
    ...
    base_url: Union[str, None] = "https://www.tiktok.com"
...
class TikTokApi:
    ...
    def __init__(self, logging_level: int = logging.WARN, logger_name: Union[str, None] = None):
        """
    ...
    async def __create_session(
        ...
        # type: ignore
        context = await self.browser.new_context(proxy=proxy, **context_options) # type: ignore
        if cookies is not None:
            ...
            await context.add_cookies(formatted_cookies)  # type: ignore
    ...
    async def create_sessions(
        ...
        proxies: Union[List, None] = None,
        sleep_after=1,
        starting_url="https://www.tiktok.com",
        context_options: dict = {},
        override_browser_args: Union[List[dict], None] = None,
        ...
        if headless and override_browser_args is None:
            override_browser_args = ["--headless=new"] # type: ignore
            headless = False  # managed by the arg
        self.browser = await self.playwright.chromium.launch(
            headless=headless, args=override_browser_args # type: ignore
        )

        await asyncio.gather(
            *(
                self.__create_session(
                    proxy=random_choice(proxies), # type: ignore
                    ms_token=random_choice(ms_tokens), # type: ignore
                    url=starting_url,
                    context_options=context_options,
                    sleep_after=sleep_after,
                    cookies=random_choice(cookies), # type: ignore
                )
    ...
    async def close_sessions(self): # type: ignore
        """
    ...
    async def make_request(
        ...
        **kwargs,
        ...
        while i < retries:
            ...
            if result == "":
                raise EmptyResponseException(result, "TikTokApi.run_fetch_script returned an empty string")
